    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm sm:text-base text-gray-600 break-words">Manage customer refund requests and approvals</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <button 
            onClick={() => { fetchRefunds(); fetchStats(); }}
            disabled={loading}
            className="flex-1 sm:flex-none flex items-center justify-center px-3 sm:px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 disabled:opacity-50 text-xs sm:text-sm"
          >
        ...
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow">
          <div className="flex items-center">
            <div className="p-1.5 sm:p-2 bg-gray-100 rounded-lg flex-shrink-0">
              <RotateCcw className="w-4 h-4 sm:w-5 sm:w-5 md:w-6 md:h-6 text-gray-600" />
            </div>
            <div className="ml-2 sm:ml-3 md:ml-4 min-w-0">
              ...
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{stats?.total || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow">
          <div className="flex items-center">
            <div className="p-1.5 sm:p-2 bg-yellow-100 rounded-lg flex-shrink-0">
              <Clock className="w-4 h-4 sm:w-5 sm:w-5 md:w-6 md:h-6 text-yellow-600" />
            </div>
            <div className="ml-2 sm:ml-3 md:ml-4 min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-600 truncate">Pending</p>
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{stats?.pending || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow">
          <div className="flex items-center">
            <div className="p-1.5 sm:p-2 bg-blue-100 rounded-lg flex-shrink-0">
              <Clock className="w-4 h-4 sm:w-5 sm:w-5 md:w-6 md:h-6 text-blue-600" />
            </div>
            <div className="ml-2 sm:ml-3 md:ml-4 min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-600 truncate">Approved</p>
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{stats?.approved || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow">
          <div className="flex items-center">
            <div className="p-1.5 sm:p-2 bg-green-100 rounded-lg flex-shrink-0">
              <CheckCircle className="w-4 h-4 sm:w-5 sm:w-5 md:w-6 md:h-6 text-green-600" />
            </div>
            <div className="ml-2 sm:ml-3 md:ml-4 min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-600 truncate">Completed</p>
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{stats?.completed || 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow">
          <div className="flex items-center">
            <div className="p-1.5 sm:p-2 bg-red-100 rounded-lg flex-shrink-0">
              <XCircle className="w-4 h-4 sm:w-5 sm:w-5 md:w-6 md:h-6 text-red-600" />
            </div>
            <div className="ml-2 sm:ml-3 md:ml-4 min-w-0">
              <p className="text-xs sm:text-sm font-medium text-gray-600 truncate">Rejected</p>
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">{stats?.rejected || 0}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="flex items-end">
            <button className="w-full flex items-center justify-center px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 text-sm">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-900 flex items-center flex-wrap gap-2">
                            ...
                            {(() => {
                              ...
                              let images: string[] = [];
                              if (request.imagesUrls) {
                                if (typeof request.imagesUrls === 'string') {
                                  try {
                                    images = JSON.parse(request.imagesUrls);
                                  } catch (e) {
                                    console.error('Failed to parse images:', e);
                                  }
                                } else if (Array.isArray(request.imagesUrls)) {
                                  images = request.imagesUrls;
                                }
                              }
                              
                              return images.length > 0 && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800" title={`${images.length} evidence image(s)`}>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              {(() => {
                // Parse images if they're a JSON string, otherwise use as-is
                let images: string[] = [];
                if (selectedRequest.imagesUrls) {
                  if (typeof selectedRequest.imagesUrls === 'string') {
                    try {
                      images = JSON.parse(selectedRequest.imagesUrls);
                    } catch (e) {
                      console.error('Failed to parse images:', e);
                    }
                  } else if (Array.isArray(selectedRequest.imagesUrls)) {
                    images = selectedRequest.imagesUrls;
                  }
                }
                ...
                return images.length > 0 && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                      ...
                      <div
                        key={index}
                        className="aspect-square rounded-lg overflow-hidden border-2 border-gray-200 hover:border-blue-500 transition-all cursor-pointer group relative"
